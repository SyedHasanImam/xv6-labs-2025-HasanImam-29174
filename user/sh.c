...
// Try to complete 'buf' in place (up to position *i)
static void
tab_complete(char *buf, int *i, int nbuf) {
  int fd;
  struct dirent de;
  char prefix[100];
  int plen = *i;

  // copy current word (before cursor)
  int start = plen-1;
  ...
  if(match_count == 1) {
    ...
    int extra = strlen(match) - len;
    if(*i + extra < nbuf) {
      memmove(buf+plen, match+len, extra+1);
      for(int j = 0; j < extra; j++)
        putc(1, match[len+j]);
      *i += extra;
    }
  } else if(match_count > 1) {
    printf("\n$ %s", buf);  // reprint prompt + current buffer
  }
}
...
getcmd(char *buf, int nbuf) {
  ...
  while(i+1 < nbuf) {
    ...
    if(c == '\n' || c == '\r') {
      ...
      tab_complete(buf, &i, nbuf);   // use the helper you added
    } else {
      buf[i++] = c;
      putc(1, c);
    }
